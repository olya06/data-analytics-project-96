...
select source, count(*), date_trunc('day', visit_date) visit_date from sessions s -- по дням
group by 1, 3
order by visit_date

select source, count(*), date_trunc('week', visit_date) visit_date from sessions s -- по неделям
group by 1, 3
order by visit_date

select source, count(*), date_trunc('month', visit_date) visit_date from sessions s  -- по месяцам
group by 1, 3
order by visit_date
